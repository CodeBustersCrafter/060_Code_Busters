...
# def extract_data_overeall_from_urls(urls):
#     all_data = []
#     for url in urls:
#         try:
...
# async def analyze_overall_content(content, openai_client,tavily_client):
...
#     prompt = f"""Analyze the following content and provide trends:

#     Main Content: {content}
#     Additional Context: {tavily_context}

#     Organize the data for plotting graphs about current voting percentages for top 4 candidates.

#     Your response should be in JSON format following Streamlit library syntax. For each graph, include a key-value pair where the key is the graph name, and the value is an object containing the data and the type of the graph.

#     - Do not include any descriptive text in the response.
#     - Need one graph for each url (bar , pie or line)(maximum number of graphs = minimum number of graphs = url count)
